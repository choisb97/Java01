...
// => 우선순위 default 값 확인 : 5 
//    단, 이전 스레드의 값을 전달 받음
//    ( 이전 스레드가 10이었으면 동일한 10이 기본 우선순위 값이 됨 )
// => 확인 : getPriority(), 변경 : setPriority()
// => 우선순위의 지정(변경)은 반드시 start전에 해야 함.
...
// ** Rabbit01 Test
// => 우선순위는 동일하고 sleepTime은 서로 다름
//    sleepTime을 Random으로 생성
class Rabbit01 extends Thread {
	...
	public void run() {
		...
		for (int i = 50; i > 0; i--) {
			...
			// ** sleep 적용
			// => Thread에 정의, Checked Exception
			//    매개변수로 sleepTime 전달 (단위는 1/1000초 )
			try {
...
//		Rabbit01 blue = new Rabbit01("Blue");
		...
		//  => sleepTime이 동일하기 때문에 결과에 우선순위의 영향이 거의 없음
