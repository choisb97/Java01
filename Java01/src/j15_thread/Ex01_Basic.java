...
// ** Thread 클래스 구현
// => Thread를 적용할 수 있는 클래스를 구현
// 1) Thread 클래스 상속
// 2) Runnable interface 구현

// ** 계층구조 : Runnable interface -> Thread 클래스
// => Thread 클래스에 Thread를 지원하는 다양한 메서드들이 정의 되어있음 
// => Thread 클래스가 Runnable을 구현함. 
//    public class Thread implements Runnable { ....   }

// => Thread 클래스의 생성자 중 Runnable을 매개변수로 하는 생성자가 있음
//	  -> Runnable을 구현한 MyThread02 생성 시 이용됨
...
// ** run() 메서드
// => thread 실행의 주체 메서드
//    생성된 thread의 main 메서드
// => thread를 통해 실행하려는 기능 (객체가 해야되는 일)을 여기에 작성함.
// => start() 메서드 호출에 의해 실행됨.
//    자신의 일을 다 마치면 (run 메서드의 종료) 자동으로 소멸됨   
// ** start() 메서드
// => Thread를 시작함 (Runnable 상태로)
...
public class Ex01_Basic {
	...
	public static void main(String[] args) {
		// 1. 생성
		// 1.1) Thread01 생성
		// => Thread를 상속했으므로 생성 시 Thread 클래스도 생성됨 
		MyThread01 t01 = new MyThread01();
		
		// 1.2) Thread02 생성
		// MyThread02 tr = new MyThread02(); // OK
		Runnable tr = new MyThread02();     // OK 
		// => Thread와 무관한 일반클래스 생성
		//    Thread에서 제공하는 메서드 등 맴버 접근불가 ( tr.start() -> XXX )
		Thread t02 = new Thread(tr);
		// => Runnable 의 참조값(인스턴스)을 이용하여 Thread를 생성해야 함
		// => Thread 클래스의 생성자 중에는 Runnable 인터페이스의 참조값(인스턴스)을 인자로 하는 생성자가 구현되어 있음				
		//    생성자 Thread(Runnable target)
		
		// Thread t02 = new Thread(new MyThread02());
		// => 불필요한 tr 인스턴스 생성 없이 바로 처리해도 됨
		...
		// 3. 결과 비교
		// => multi thread는 실행되지 않고 main이 일반 메서드 호출 실행.  
		//    thread 적용 없이 순서대로 실행 
//		t01.run();
//		t02.run();
