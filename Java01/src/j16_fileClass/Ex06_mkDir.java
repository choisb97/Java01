...
public class Ex06_mkDir {
	...
	public static void main(String[] args) {
		...
		// ** mkDir
		File f2 = new File("d:\\MTest\\MyWork\\ppp") ; // 새로만들려는 디렉토리
		if (!f2.exists()) { // 새로만들려는 디렉토리 의 존재확인, 없으면 만들기
			f2.mkdir() ; 
		}
		// File f2 = new File("d:\\MTest1\\MyWork1\\ppp") ;
		// => mkdir, rename 시에 새로 만들려는 디렉토리의 Parent가 존재하지 않으면,  
		...
		listDirectory(new File("d:\\MTest\\MyWork")); // d:\\MTest\\MyWork의 목록 출력
		f2.renameTo(new File("d:\\MTest\\MyWork\\aaa")); // aaa -> bbb로 이름 변경
		listDirectory(new File("d:\\MTest\\MyWork")); // bbb로 변경한 후 리스트 출력
	} // main
} // class
