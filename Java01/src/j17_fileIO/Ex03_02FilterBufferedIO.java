...
//    객체 내부에 버퍼를 가지고 있어서 일단 버퍼에 채운 후 버퍼를 단위로 I/O 처리 
...
public class Ex03_02FilterBufferedIO {
	...
	public static void main(String[] args) throws IOException {
		...
		// => 입력스트림을 생성하고, 필터스트림 BufferedInputStream과 연결
		BufferedInputStream in = new BufferedInputStream(new FileInputStream("d:\\MTest\\sql1209.sql"));
		
		// 2) BufferedOutputStream 생성
		// => 출력스트림을 생성하고, 필터스트림 BufferedOutputStream과 연결
		BufferedOutputStream out = new BufferedOutputStream(new FileOutputStream("bufferedTest.txt"));
		...
		try {
			...
			while(true) {
				data = in.read();
				// ** BufferedInputStream
				// => FileInputStream으로부터 파일의 자료를 가져다 버퍼에 채운 후
				...
				// => FileOutputStream으로부터 자료를 가져다 버퍼에 채운 후
				//    write() 메서드에서 이 버퍼의 자료를 파일에 write 함.
				// => 주의사항 (출력스트림에만 해당)
				//	  버퍼스트림의 데이터가 파일에 저장되지 않은 상태에서 오류로 종료된다면 	
				//    데이터가 손상될 수 있다.
